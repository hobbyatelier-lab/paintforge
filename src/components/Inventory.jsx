...
export default function Inventory({ user }) {
  ...
  const savePaint = useCallback(async (id, patch) => {
    ...
    const current = {
      ...
      extras: extras[id] || 0, target_count: targets[id] || 0, ...patch,
    }
  ...
  function exportOwned() {
    ...
    for (const [key, label] of Object.entries(SECTION_LABELS)) {
      ...
      const rel = COLORS[key].filter(c => checked[c.id] || mySet[c.id] || extras[c.id])
    ...
    lines.push(`Owned: ${ownedCount} / ${allPaints.length}  |  My Set: ${setOwned}/${setTracked.length}`)
    showExportModal('Inventory Export', lines.join('\n'))
  }
  ...
  function exportShoppingList() {
    ...
    for (const [key, label] of Object.entries(SECTION_LABELS)) {
      ...
      const needed = COLORS[key].filter(c => mySet[c.id] && !checked[c.id])
        .sort((a,b) => parseFloat(a.id) - parseFloat(b.id))
      ...
      needed.forEach(c => {
        ...
        const qty = t > 0 ? t+1 : 1
        const note = t > 0 ? `  (1 use + ${t} backup${t>1?'s':''})` : ''
    ...
    for (const [key, label] of Object.entries(SECTION_LABELS)) {
      if (!COLORS[key]) continue
      const needed = COLORS[key].filter(c => checked[c.id] && (targets[c.id]||0)>0 && (extras[c.id]||0)<(targets[c.id]||0))
        .sort((a,b) => parseFloat(a.id) - parseFloat(b.id))
      if (!needed.length) continue
      lines.push(`  ${label}`)
      needed.forEach(c => {
    ...
    showExportModal('Shopping List', lines.join('\n'))
  }
      ...
      {showExport && (
        <div style={{ position:'fixed', inset:0, background:'#000a', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div style={{ background:'#1e1e2e', borderRadius:12, padding:24, width:'100%', maxWidth:480, border:'1px solid #3a3a4a' }}>
            ...
            <p style={{ margin:'0 0 12px', fontSize:12, color:'#888' }}>Copied to clipboard. Select all → save as .txt for backup.</p>
        ...
        <div style={{ maxWidth:680, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <button onClick={() => setShowHowToUse(true)} title="How to Use" style={{ width:24, height:24, borderRadius:'50%', border:'1px solid #3a3a4a', background:'#2a2a3a', color:'#888', fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700 }}>?</button>
          ...
          <div style={{ marginBottom:12 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
              ...
              <span style={{ fontSize:12, color:'#aaa' }}>
                ...
                <span style={{ color:'#554' }}> · {setPct}%</span>
              </span>
            </div>
            <div style={{ height:5, background:'#2e2e3e', borderRadius:3, overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginBottom:5 }}>
            ...
            <div style={{ marginLeft:'auto', display:'flex', gap:5 }}>
              <button onClick={()=>setShowBrandFilter(true)} style={{
                padding:'4px 10px', borderRadius:20, cursor:'pointer', fontSize:11, fontWeight:600,
                border: hiddenCount > 0 ? '1px solid #e94560' : '1px solid #3a3a5a',
                background: hiddenCount > 0 ? '#2a0a10' : 'transparent',
                color: hiddenCount > 0 ? '#e94560' : '#7070a0',
              }}>Brands{hiddenCount > 0 ? ` (${hiddenCount})` : ''}</button>
  ...
  return (
    <div style={{ marginBottom:24 }}>
      <div onClick={onToggleCollapse} style={{
        ...
        paddingBottom: isCollapsed?0:6, borderBottom: isCollapsed?'none':`2px solid ${accent}33`,
  ...
  return (
    <div style={{
      ...
      <div style={{ display:'flex',gap:2,flexShrink:0 }}>
        {[1,2,3,4,5].map(n=>(
          <button key={n} onClick={()=>setExtraCount(color.id,n)} title={`${n} backup${n>1?'s':''} owned`} style={{ width:9,height:9,borderRadius:'50%',border:'none',cursor:'pointer',padding:0,background:n<=extraCount?'#f07030':'#2a2a3a' }} />
        ))}
      </div>
      <div style={{ display:'flex',gap:2,flexShrink:0,marginLeft:3,paddingLeft:3,borderLeft:'1px solid #2a2a3a' }}>
        {[1,2,3,4,5].map(n=>(
          <button key={n} onClick={()=>setTargetCount(color.id,n)} title={`Target ${n} backup${n>1?'s':''}`} style={{ width:9,height:9,borderRadius:'50%',cursor:'pointer',padding:0,background:n<=targetCount?'#20a080':'transparent',border:`1px solid ${n<=targetCount?'#20a080':'#3a3a4a'}` }} />
        ))}
      </div>
    </div>
  )
