...
export default function Inventory({ user }) {
  ...
  const savePaint = useCallback(async (id, patch) => {
    ...
    const current = {
      ...
      in_my_set:    !!mySet[id],
      extras:       extras[id]  || 0,
      target_count: targets[id] || 0,
      ...patch,
    }
    const isEmpty = !current.owned && !current.in_my_set
                 && current.extras === 0 && current.target_count === 0
    if (isEmpty) {
      await supabase.from('user_paints').delete()
        .eq('user_id', user.id).eq('paint_id', id)
    } else {
  ...
  const setTotal    = setTracked.length
  const setPct      = setTotal > 0 ? Math.round((setOwned / setTotal) * 100) : 0
  ...
  return (
    <div style={{ background: '#141414', minHeight: '100vh', fontFamily: "'Inter', system-ui, sans-serif", color: '#e8e8e8' }}>
      ...
      <div style={{
        ...
        borderBottom: '1px solid #2e2e3e', padding: '16px 20px',
        ...
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {saving && <span style={{ fontSize: 11, color: '#555', marginLeft: 4 }}>saving…</span>}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12, color: '#555' }}>{user.email}</span>
              <button onClick={handleSignOut} style={{
                padding: '4px 10px', borderRadius: 6, border: '1px solid #3a3a4a',
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
              ...
              <span style={{ fontSize: 12, color: '#aaa' }}>
                ...
                <span style={{ color: '#444' }}>/{setTotal}</span>
              </span>
            </div>
            <div style={{ height: 5, background: '#2e2e3e', borderRadius: 3, overflow: 'hidden' }}>
          ...
          <input
            ...
            style={{
              ...
              background: '#141414', border: '1px solid #2e2e3e',
...
function ColorRow({ color, isChecked, inMySet, extraCount, targetCount, toggleOwned, toggleMySet, setExtraCount, setTargetCount }) {
  ...
  return (
    <div style={{
      ...
      {/* My Set */}
      <button onClick={() => toggleMySet(color.id)} title="Mark as My Set" style={{
        width: 15, height: 15, borderRadius: 3, border: 'none', cursor: 'pointer', flexShrink: 0,
        background: inMySet ? '#9060d0' : '#2a2a3a',
      ...
      {/* Owned */}
      <button onClick={() => toggleOwned(color.id)} title="Mark as Owned" style={{
        width: 15, height: 15, borderRadius: 3, border: 'none', cursor: 'pointer', flexShrink: 0,
        background: isChecked ? '#4caf50' : '#2a2a3a',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
      ...
      <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
        {[1,2,3,4,5].map(n => (
          <button key={n} onClick={() => setExtraCount(color.id, n)} title={`${n} backup${n>1?'s':''} owned`} style={{
            width: 9, height: 9, borderRadius: '50%', border: 'none', cursor: 'pointer', padding: 0,
            background: n <= extraCount ? '#f07030' : '#2a2a3a',
          }} />
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 2, flexShrink: 0, marginLeft: 3, paddingLeft: 3, borderLeft: '1px solid #2a2a3a' }}>
        {[1,2,3,4,5].map(n => (
          <button key={n} onClick={() => setTargetCount(color.id, n)} title={`Want ${n} backup${n>1?'s':''}`} style={{
            width: 9, height: 9, borderRadius: '50%', cursor: 'pointer', padding: 0,
            background: n <= targetCount ? '#20a080' : 'transparent',
            border: `1px solid ${n <= targetCount ? '#20a080' : '#3a3a4a'}`,
          }} />
        ))}
      </div>
    </div>
