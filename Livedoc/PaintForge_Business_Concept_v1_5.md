...
v1.5 · July 10, 2026 (PM) · Agustín Medina --- launch-eve close-out:
Tamiya Spray sampled (+TS-102, catalog 6,260), anvil provenance system
shipped, ΔE explainer rewritten, branded loading, changelog synced.
(Supersedes v1.4 consistency pass and v1.3 rev U.)
...
> database (6,260 entries at launch, 13 brands). Owned / My Set /
...
> **▸** Multi-brand paint inventory: 6,260 entries across 13 brands.
...
covering 6,260 entries across 13 brands. Three collapsible levels:
...
> 1,000-row pages until exhausted --- the 6,260-entry database will
...
6,260 entries across 13 brands. Hex color swatch data for the majority
...
> **✦** *STATUS --- July 10, 2026 PM (launch-eve close-out): DATA ---
> Tamiya Color Spray fully sampled from manufacturer digital chips: 101
> hexes updated + 1 new entry inserted (TS-102 Cobalt Green, previously
> missing from the catalog). CURRENT CATALOG TOTAL: 6,260 entries.
> Total PaintForge-sampled entries: 559 (was 458); canonical sampling
> record committed to repo (sampling_record.md) --- rule: check the
> record before sampling, never sample a line twice. True Metal Shade
> section reclassified metallic → wash (20 rows; they are shades, not
> coverage metallics). INCIDENT + FIX: the LAB recompute webhook did
> not fire for the INSERTed row (TS-102) and did not respond to a no-op
> UPDATE nudge; LAB values written manually (validated pipeline).
> POST-LAUNCH TICKET: check Edge Function event configuration (INSERT +
> UPDATE) and invocation logs --- every future catalog addition is
> born LAB-less until fixed. SECOND INCIDENT: src/main.jsx accidentally
> deleted during file reorganization; restored from git history ---
> lesson recorded: git never forgets, rebuild-from-memory risks losing
> PostHog init. SHIPPED: anvil provenance system (provenance.jsx ---
> SAMPLED_SECTIONS registry + AnvilBadge component; badge on DetailPopup
> swatch 18px, IrisMatch comparison swatches ~21px, deliberately NOT on
> tiny list swatches; inline variant in the popup provenance line;
> tooltip: "Sampled by PaintForge from the manufacturer's digital
> chips"). HowToUse swatch legend expanded to six visual states (true /
> approximate / no data ? / not applicable --- / colorshift ~ /
> PaintForge-sampled anvil) with matching descriptions; misplaced My
> Set tip relocated under the header-counters explanation. ΔE tooltip
> fully rewritten (plain + science, CIEDE2000/Sharma provenance, L·a·b
> axis meanings, grade table, honesty rule) with readable contrast
> tokens (body #B8CFCF, muted #7A9595 --- footer greys banned from
> sentence text). DetailPopup: hex value row removed (redundant with
> swatch); manufacturer code retained. LoadingScreen component shipped
> (branded splash, logo from src/assets, breathe animation) for both
> load gates. Changelog v0.1.1 rewritten as launch-eve polish entry
> (catalog 6,260, sampled 559, provenance marks, legend, explainer,
> pages). LAUNCH STATE: all build items closed; remaining = deploy
> verification + Friday.*
